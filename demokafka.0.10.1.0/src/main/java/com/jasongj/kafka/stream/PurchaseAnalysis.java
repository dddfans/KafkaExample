...
//		System.setProperty("ROCKSDB_SHAREDLIB_DIR", "C:/Users/Administrator/AppData/Local/Temp/");
		...
		KTable<String, User> userTable = streamBuilder.table(Serdes.String(), SerdesFactory.serdFrom(User.class), "user6", "users-state-store");
...
//		itemTable.toStream().foreach((String itemName, Item item) -> System.out.printf("Item info %s-%s-%s-%s\n", item.getItemName(), item.getAddress(), item.getType(), item.getPrice()));
		orderStream
				...
				.map((String userName, OrderUser orderUser) -> new KeyValue<String, String>(orderUser.itemName, orderUser.getUserName() + ":" + orderUser.getItemName() + ":" + orderUser.getTransactionDate()))
//				.map((String userName, OrderUser orderUser) -> KeyValue.<String, Double>pair(orderUserItem.gender, (Double)))
//				 
...
//				.groupByKey(Serdes.String(), SerdesFactory.serdFrom(OrderUser.class))
				//.reduce((String v1, String v2) -> v1 + v2, "gender-amount-state-store")
				.to("my-result");
